...
def main(seed):
    ...
    par_nuts.to_pickle('./parameters/par_nuts_sep_seed{}'.format(seed))
    np.save('./parameters/time_nuts_sep_seed{}'.format(seed),end_nuts-start_nuts)
    ...
    par_advi.to_pickle('./parameters/par_advi_sep_seed{}'.format(seed))
    np.save('./parameters/time_advi_sep_seed{}'.format(seed),end_advi-start_advi)
...
def main_plot():
    ...
    #process NUTS data and save
    par_nuts = pd.DataFrame(fit_nuts.to_dataframe())
    par_nuts.to_pickle('./parameters/par_nuts_sep_plot')
    np.save('./parameters/time_nuts_sep_plot',end_nuts-start_nuts)
    ...
    params_advi = fit_advi['sampler_params']
    par_advi = pd.DataFrame(dict(zip(param_names_advi,params_advi)))
    par_advi.to_pickle('./parameters/par_advi_sep_plot')
    np.save('./parameters/time_advi_sep_plot',end_advi-start_advi)

if __name__=='__main__':
    setup_model()
    seed = np.arange(100,130,1)
    Parallel(n_jobs=10, backend= 'loky')(delayed(main)(seed[i]) for i in tqdm(range(30)))

    #Posterior samples for plot
    main_plot()
