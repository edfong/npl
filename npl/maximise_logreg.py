...
def sampleprior(x,N_data,D_covariate,T_trunc): #sample prior pseudo-samples
    ind_x = np.random.randint(low = 0, high = N_data, size = T_trunc)  #sample x indices with replacement
    x_prior = x[ind_x]
    y_prior = bernoulli.rvs(0.5, size = T_trunc)
...
def maximise(y,x,N_data,D_covariate,alph_conc,T_trunc,weights,beta_init,a,b,gamma,R_restarts): #maximize weighted loss
    if alph_conc!= 0:  #concatenate prior pseudo-samples
        y_prior, x_prior = sampleprior(x,N_data,D_covariate,T_trunc)
